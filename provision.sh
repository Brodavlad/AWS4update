...
aws ec2 create-key-pair --key-name MyKeyPairLB --query 'KeyMaterial' --output text > ./MyKeyPairLB.ppk

# Modify permissions
chmod 400 MyKeyPairLB.ppk

# Retrieve ami_ID
IMAGE_ID=`aws ec2 describe-images --owners 695200478063 --query Images[0].ImageId --output text`
...
aws autoscaling create-auto-scaling-group \
    --auto-scaling-group-name AutoScalingGroup2 \
    --instance-id $INSTANCE_1_ID \
    --min-size 2 \
    --max-size 2 \
    --target-group-arns $TG_ARN 

# Update health settings every 15sec
...
aws autoscaling describe-auto-scaling-groups --auto-scaling-group-name AutoScalingGroup2
